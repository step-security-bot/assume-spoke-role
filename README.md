...
1. Once you've done everything above, you can use `go get`.
    ...
    ```bash
    go install github.com/northwood-labs/assume-spoke-role
    ```
...
# Using your local credentials (e.g., sys-robot), assume a role in the "HUB"
...
func main() {
    ...
    roleCredentials, _, err := hubspoke.GetSpokeCredentials(&hubspoke.SpokeCredentialsInput{
        ...
        ExternalID:     "abc123",      // Only if you need this.
...
Using the "robot" process example, let's follow-through with creating an IAM role to assume, and call it `robot-hub-role`. This is an IAM role which will grant access to your user for assuming the "spoke" role in every account which has that identically-named role.
